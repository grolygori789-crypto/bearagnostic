(() => {
  ...
  const BUILD = Number(CONFIG.build || 6);
  ...
  function openInfo(kind) {
    if (kind === 'about') {
      ...
      byId('infoContent').innerHTML = `<div class="about-sheet-brand"><img src="./assets/icons/app-icon-192.png" alt="" width="58" height="58"><div><strong>Bearagnostic</strong><span>v${escapeHTML(CONFIG.appVersion || '0.1.5')} · Build ${BUILD}</span><span>Benedict Interactive · Bangkok, Thailand</span></div></div><p>${escapeHTML(t('aboutPrivacyNote'))}</p>`;
  ...
  function safeDiagnostics() {
    ...
    return [
      `App: Bearagnostic`, `Version: ${CONFIG.appVersion || '0.1.5'}`, `Build: ${BUILD}`, `Language: ${currentLanguage}`,
  ...
  function registerServiceWorker() {
    ...
    window.addEventListener('load', () => navigator.serviceWorker.register('./sw.js?v=6',{scope:'./'}).catch(()=>{}), { once:true });
  }
  function boot() {
